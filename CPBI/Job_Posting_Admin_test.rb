...
class JobPostingAdmin < Test::Unit::TestCase
  ...
  def test_A_Search_Job_Posting_BackEnd
    ...
    if get_total_job_number.scan(/Total number of records found: [0]/).empty?
      ...
      @cpbi_backend.verify { assert_equal(job_posting_number, @driver.find_element(:id => 'ctl00_MPContent_ctlJobPostingsList_grdJobPostings_ctl02_lnkInvoice').text, 'Number of record does not match') }
    else
